...
neighThres = 45       # minsup: minimum number of ST neighbors threshold
...
indexArr = np.load('outputs/ST_IB_4/sim_0/clustIndex_' + str(neighThres) + '_' + str(percentile) + ".npy")

#load observed odds ratio (at 95 percentile)
obsOddsRatio = np.loadtxt('outputs/ST_IB_4/sim_0/odds_ratio_' + str(neighThres) + ".txt", delimiter = ',', usecols=-1)[5]
...
outFile = open(outDir + os.sep + 'cluster_significance_' + str(neighThres) + "_" + str(percentile) + ".txt", "w")
